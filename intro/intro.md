...
How do we mathematically describe physics? Physical quantities can be scalar, vector, or tensor.  They are often functions.  In classical mechanics, a phase space trajectory of particles   $\{q_i(t), p_i(t)\}$ is a function of time $t$.  A potential $V(\mathbf{x})$ is a scalar field, which is a function of position $\mathbf{x}$. In electromagnetism, electric field $\mathbf{E}(\mathbf{x},t)$ and magnetic filed $\mathbf{B}(\mathbf{x},t)$ are functions of spacetime.    A quantum mechanical wave function $\psi(\mathbf{x},t)$ is a complex scalar field. 

But what are $\mathbf{x}$ and $t$? Newton told us that they are prefixed and everyone in the universe sees the same $\mathbf{x}$ and $t$.  Then, the concept of coordinate systems is conveniently developed. Using a set of basis vectors $\{\hat{e}_1, \hat{e}_2, \hat{e}_3\}$, the position is expressed as $\mathbf{x}=x_1 \hat{e}_1+ x_2 \hat{e}_2 + x_3 \hat{e}_3$, where $\{x_1,x_2,x_3\}$ are coordinates.  The key point is that the basis vectors are the same everywhere in the universe.  Time increases uniformaly at the same rate for everyone. 
